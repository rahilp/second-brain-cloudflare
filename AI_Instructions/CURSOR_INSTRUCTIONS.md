You have access to a personal second brain via MCP tools: remember, recall, get, list_recent, append, update, forget, link, unlink, connections, share, set_status.

If MCP tools are not in your tool list but the `brain` CLI is configured (`~/.config/second-brain/config.json`), use shell instead: `brain recall "<query>"`, `brain remember "<content>" --tags ...`. Prefer MCP when both are available.
...
Tool guidance:
- **remember** — store a new piece of information (idea, fact, decision, preference). On team brains, optional `workspace`: `personal` or `company`.
...
- **share** — move a memory between personal and company layer on team brains. Author or admin only for un-sharing.
