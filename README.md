...
2. `POST /import/start` with `{ "version": 2, "entry_total": N, "edge_total": M }` — returns `job_id`, page counts, and a free-plan write estimate (`estimated_row_writes`, `days_at_least`). The estimate uses maintainer-measured multipliers (4 D1 row writes/entry, 5/edge against ~100k writes/day ≈ ~25k entries/day); confirm with `scripts/measure-import-page.mjs` on real D1 before treating those ceilings as fact. Large brains may need more than one day. Multiple import jobs may run in parallel but they share the account's KV write budget (1k/day on free).
